...
class ProblemFactoryTest(unittest.TestCase):
    def test_create_addition(self):
        ...
        sample_problems = [problem_factory.create_addition(min_value, max_value) for _ in range(500)]

        self.assertTrue(all([min_value <= int(prob.op1) <= max_value for prob in sample_problems]))
        self.assertTrue(all([min_value <= int(prob.op2) <= max_value for prob in sample_problems]))
